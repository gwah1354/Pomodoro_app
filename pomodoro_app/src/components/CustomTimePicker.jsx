...
function CustomTimePicker({ isOpen, onClose, onSelect }) {
  ...
  const handleScroll = () => {
    ...
    setSelectedMinutes(minutes[clampedIndex])
  ...
  const handleItemClick = (minutes) => {
    setSelectedMinutes(minutes)
    const itemHeight = 40
    const index = minutes - 1
    if (scrollRef.current) {
      scrollRef.current.scrollTo({
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-[#F5F1E8] rounded-2xl p-6 w-80 shadow-2xl"
        ...
        >
          ...
          <div className="relative h-64 mb-6">
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="h-12 w-full bg-[#E8E2D6] rounded-lg opacity-50"></div>
            </div>
            
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
              <div className="h-12 w-full border-2 border-[#C8B6A6] rounded-lg"></div>
            </div>
            
            <div
              ref={scrollRef}
              onScroll={handleScroll}
              className="h-full overflow-y-auto scrollbar-hide py-28"
              style={{
                scrollbarWidth: 'none',
